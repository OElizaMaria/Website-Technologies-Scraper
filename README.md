...
a few categories to match against(scripts, links, meta etc). Afterwards we enter the technology detection logic (detect_tech function)
where we have a fast guard that checks if the tech name is in the file, if not it doesn't bother to check the other fingerprints. If it
...
The code is build to be multi-threaded so it handles the large dataset  to be analyzed. 
For multi-threading I used the asyncio and aiohttp library, limiting at 50 threads.

The Dictionary Builder: We scrape top 500 technologies from a website and look for them in the wappalyzer dictionary. To properly work
with our scraper and to run smoothly, we clean the strings to keep them in their simplest form. This removes some fingerprints, but 
...
my own implementation of the web scraper. I first built the code of the scraper on a small hand-written dictionary, however even than I noticed
